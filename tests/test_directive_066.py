...
import jbot_agent as agent


def test_task_proposal_flow():
    with patch("jbot_tasks.get_memory_client") as mock_nb:
...
def test_token_tracking_pipeline(tmp_path):
    ...
    project_dir.mkdir()
    (project_dir / ".jbot").mkdir()
    (project_dir / ".jbot/agents.json").write_text(
        json.dumps({"lead": {"role": "Lead"}})
    )
    (project_dir / ".jbot/queues").mkdir()

    # Mock AI interface to return stats
    mock_stats = {"total_tokens": 1234, "prompt_tokens": 1000, "completion_tokens": 234}

    with patch(
        "jbot_agent_interface.GeminiInterface.run", return_value=(0, mock_stats)
    ):
        with patch("jbot_core.ensure_single_user"):
            with patch("jbot_core.switch_to_develop"):
                with patch("jbot_agent.assemble_context", return_value="prompt"):
                    # Run agent
                    agent.run_agent(
                        name="lead",
                        role="Lead",
                        project_dir=str(project_dir),
                        prompt_file="jbot_prompt.txt",
                        cli_type="gemini",
                        cli_bin="gemini",
                    )

    # Verify stats recorded in queue JSON
    queue_file = project_dir / ".jbot/queues/lead.json"
